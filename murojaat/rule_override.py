...
def apply_rule_override(text: str, prediction: str, probs: list) -> tuple:
    text_norm = normalize_text(text)  # make text all lovercase
    ...
    for organ, rules in RULES.items():
        ...
        for kw in rules["hard"]:
            if phrase_in_text(kw, text_norm):     # was: kw.lower() in text_lower
                hard_fired.append(organ)
                probs[idx[organ]]["prob"] = max(probs[idx[organ]]["prob"], HARD_BOOST)

        # Soft keywords: 
        soft_hits = sum(1 for kw in rules["soft"] if phrase_in_text(kw, text_norm))
        if soft_hits > 0:
