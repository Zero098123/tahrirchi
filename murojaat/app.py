...
def predict():
    ...
    # ── Step 2b: Secondary recommendations (NEW) ───────────────────────────
    # probs is already sorted descending by apply_rule_override, so probs[0]
    # is guaranteed to be the top organ here.
    top_prob = probs[0]['prob']
    recommendation_threshold = top_prob / 2
    recommended_organs = [
        p for p in probs if p['prob'] >= recommendation_threshold
    ]
    ...
    if total_score > 0:
        ...
        detected_categories = []
    print("DEBUG recommended_organs:", recommended_organs)
    return render_template(
        ...
        probabilities=probs,
        recommended_organs=recommended_organs,   # NEW — pass to template
        original_text=text,
