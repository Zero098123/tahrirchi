...
def identify_multiple_problems(text: str, threshold: float = 0.10) -> dict:
    ...
    for cat, cfg in CATEGORIES.items():
        must    = _phrase_hits(text, cfg["must"])
        support = _phrase_hits(text, cfg["support"])
        penalty = _phrase_hits(text, cfg["penalty"])
    ...
    keywords = [t for t in tokens if len(t) > 2 and t.lower() not in UZ_STOPWORDS]
